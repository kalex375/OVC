create or replace package ora_ver.P_OVC_DIFF is
  /*
  ...
  */
  ...
  --Сравнить массивы исходников
  procedure Compare(p_sour_1 in t_source_table, 
                    p_sour_2 in t_source_table,
                    p_ignore_case in varchar2,
                    p_ignore_leading_space in varchar2,
                    p_ignore_trailing_space in varchar2);
...
create or replace package body ora_ver.P_OVC_DIFF is
...
procedure Compare(p_sour_1 in t_source_table, 
                  ...
                  p_ignore_trailing_space in varchar2)
is
  m_ints_1 p_ovc_diff.TIntArray;
  m_ints_2 p_ovc_diff.TIntArray;
  
begin
  select
     p_ovc_str_utils.hash_string(s.text,p_ignore_case, p_ignore_leading_space, p_ignore_trailing_space) hash_string
  bulk collect into m_ints_1   
  from
     table( cast(p_sour_1 as t_source_table)) s;

  select
     p_ovc_str_utils.hash_string(s.text,p_ignore_case, p_ignore_leading_space, p_ignore_trailing_space) hash_string
  bulk collect into m_ints_2   
  from
     table( cast(p_sour_2 as t_source_table)) s;
  
  Compare(m_ints_1,m_ints_2);   
