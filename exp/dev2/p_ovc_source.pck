create or replace package ora_ver.P_OVC_SOURCE is
  /*
  ...
  */
  ...
  --Возвращает исходные коды изменения
  function get_change_source_array(p_change_id ovc_change_object.id%type) return t_source_table;

  --Возвращает исходные коды изменения в clob
  function get_change_source_clob(p_change_id ovc_change_object.id%type) return clob;

  --Возвращает исходные коды объекта их БД(текущие)
  function get_db_source_array(p_type all_source.type%type,
                               p_owner all_source.owner%type,
                               p_name all_source.name%type) return t_source_table;

  --Возвращает исходные коды объекта их БД(текущие) в clob
  function get_db_source_clob(p_type all_source.type%type,
                              p_owner all_source.owner%type,
                              p_name all_source.name%type) return clob;
  
  -- Функции для внутреннего использывания                            
...
create or replace package body ora_ver.P_OVC_SOURCE is
...
begin
  ...
  loop
    ...
    m_table.extend;
    m_table(m_nth) := t_source_line(m_nth, dbms_lob.substr(p_clob,m_amount,m_offset),0);
    m_offset := m_ins+1;
  ...
  if m_str is not null then
    m_table.extend;  
    m_table(m_nth) := t_source_line(m_nth, m_str,0);
  end if;    
  return m_table;
end;
...
begin
  ...
  open c_prog_source for select 
                         ...
                         from
                           ovc_change_source s
                         where
                           s.change_object_id = p_change_id
                         order by s.line;
...
is
begin
  return array_to_clob(get_change_source_array(p_change_id));
end;                                                                  
...
begin
  ...
  m_obj_type_info := p_ovc_dbobject.get_object_type_info(p_type);
...
begin                             
  m_func_name := p_ovc_dbobject.get_object_function(p_type);
  
  if m_func_name is not null then
    execute immediate 'begin
                        :p_result := '||m_func_name||'(p_type => :p_type,
                                                       p_owner => :p_owner,
                                                       p_name => :p_name);
                       end;' using out m_source_array, 
                                       p_type,
                                       p_owner,
                                       p_name;
  end if;
  
  return m_source_array;
end;
...
is
begin
  return array_to_clob(get_db_source_array(p_type,p_owner,p_name));
end;                                                                  
