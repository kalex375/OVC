create or replace package ora_ver.P_OVC_STR_UTILS is
  ...
  */
  ...
  function hash_string(p_text in varchar2,
                       p_ignore_case in varchar2,
                       p_ignore_leading_space in varchar2,
                       p_ignore_trailing_space in varchar2) return pls_integer;

  --Преобразует недопустимые символы в спец. символы
  function encode_web_str(s varchar2) return varchar2;
                       
end P_OVC_STR_UTILS;
/

create or replace package body ora_ver.P_OVC_STR_UTILS is
...
begin
  ...
  if p_ignore_case = 'T' then
    m_text := nls_upper(p_text);
  else 
    m_text := p_text; 
  end if;
  
  if p_ignore_trailing_space = 'T' then
    m_text := rtrim(m_text);  
  end if;

  if p_ignore_leading_space = 'T' then
    m_text := ltrim(m_text);  
  end if;
      
  return dbms_utility.get_hash_value(m_text, 1, 65536);
end;                     
