create or replace package ora_ver.P_OVC_FILTER is
  /*
...
create or replace package body ora_ver.P_OVC_FILTER is

-- Получить тип следящего фильтра
...
begin
  ...
  if m_type is null then 
    p_ovc_exception.raise_common_exception('Не найден фильтр в группе ID=%s',p_id);
  end if;
...
begin
  if p_id is null then
    p_ovc_exception.raise_common_exception('Не задан ID фильтра');
  end if;
...
begin
  if p_id is null then
    p_ovc_exception.raise_common_exception('Не задан ID фильтра в группе');
  end if;
  ...
  if get_change_filter_type(p_id) = 'PROJECT' and p_project_id is null then
    ...
    p_ovc_exception.raise_common_exception('Не задан ID шаблона ревизии.');
    
  elsif get_change_filter_type(p_id) = 'SYSTEM' and (p_project_id is not null or p_revision_template_id is not null) then
    p_ovc_exception.raise_common_exception('Тип фильтра не может быть изменен.');
  end if;
