create or replace package P_OVC_HTTP is
...
/
create or replace package body P_OVC_HTTP is
...
begin
  ...
  m_ctxh := 
    dbms_xmlgen.newcontextFromHierarchy(
      'select
        level,
        xmlelement("name",xmlattributes(id as "code", id as "id", name as "text", level as "level", icon as "iconCls", expanded as "expanded", real_id as "real_id", node_type as "node_type"))
      from
      (
       select
         to_char(n.id) id,
       ...
       from
        ovc_project n,
        (select ''Object'' name, ''o_'' code, ''icon_folder_open'' icon, ''object_list'' node_type from dual
         union
         select ''Filter'' name, ''f_'' code, ''icon_folder_open'' icon, ''filter_list'' node_type from dual
         union
         select ''Revision'' name, ''r_'' code, ''icon_folder_open'' icon, ''revision_list'' node_type from dual) f

       union all
       select
         ''oi_'' || to_char(po.id) id,
         ''ot_'' ||ot.id||''_''||to_char(po.project_id) project_id,
         po.obj_name,
         ot.icon icon,
         ''false'' expanded,
         to_char(po.id) real_id,
         ''object'' node_type         
       from  
       ...
       select
         ''ot_'' || to_char(ot.id)||''_''||to_char(po.project_id) id,
         ''o_'' || to_char(po.project_id) project_id,
         po.obj_type,
       ...
       from 
         --ovc_project, 
         (select distinct p.project_id, p.obj_type from ovc_project_object p) po,
         ovc_object_type ot
       where
         po.obj_type = ot.type  
       ...
       select
         ...
         ''icon_filter'' icon,
         ''false'' expanded,
         to_char(s.id) real_id,
         ''filter'' node_type                  
       from
         ovc_filter_set s,
         ovc_filter_template t
       where
         s.filter_id = t.id and
